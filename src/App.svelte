...
<script>
  ...
  async function handleMoveSelect(moveKey) {
    ...
    // Log AI thinking
    addLogMessage(
      `${battleState.fighter2.name} is analyzing ${DIFFICULTY.simulationCount} possible battle outcomes...`,
    );
    ...
    addLogMessage(
      `${battleState.fighter2.name} selected ${battleState.fighter2.moves[aiMove].name} (${Math.round(winRate * 100)}% win rate).`,
    );
  ...
  function executeBattleTurn(playerMoveKey, aiMoveKey) {
    ...
    if (newBattleState.battleOver) {
      ...
      // Log battle end
      addLogMessage(
        `${newBattleState.winner === "fighter1" ? battleState.fighter1.name : battleState.fighter2.name} won the battle!`,
      );
    } else {
      // Reset for next turn
      gameState = GAME_STATES.WAITING_FOR_PLAYER;
  ...
  battleEvents.on(BATTLE_EVENTS.RECOIL_DAMAGE, (event) => {
    addLogMessage(
      `${event.fighter.name} took ${event.recoilDamage} recoil damage!`,
    );
  });

  battleEvents.on(BATTLE_EVENTS.MOMENTUM_BONUS, (event) => {
    addLogMessage(
      `${event.fighter.name} gained ${event.bonusDamage} bonus damage from momentum!`,
    );
  });
</script>
    ...
    <div class="bg-gray-800 p-6 rounded-lg shadow-lg">
      ...
      {#if battleState}
        ...
        <div class="mb-4 bg-white text-black p-3 rounded-md shadow">
          <div class="font-bold">
            ...
            {:else if gameState === GAME_STATES.BATTLE_OVER}
              Battle over! {battleState.winner === "fighter1"
                ? "You won!"
                : "AI won!"}
            {/if}
          </div>
        ...
        <div class="grid md:grid-cols-2 gap-4 mb-4">
          <!-- AI Fighter -->
          <div
            class="p-4 rounded-md mb-4 shadow-md bg-red-50 border border-red-300 text-black"
          >
            <div class="flex justify-between items-center mb-2">
              <h3 class="font-bold text-lg">{battleState.fighter2.name}</h3>
              <span class="text-sm"
                >HP: {battleState.fighter2.hp}/{battleState.fighter2
                  .maxHp}</span
              >
            </div>
            ...
            <div class="w-full bg-gray-200 h-4 mb-3 rounded-full">
              <div
                ...
                style="width: {Math.max(
                  ...
                  Math.min(
                    100,
                    (battleState.fighter2.hp / battleState.fighter2.maxHp) *
                      100,
                  ),
                )}%"
              ></div>
            ...
            {#if battleState.battleOver}
              <div class="grid gap-2">
                {#each Object.entries(battleState.fighter2.moves) as [moveKey, move]}
                  <div
                    class="p-2 text-white rounded-md w-full mb-2 transition-all bg-red-500"
                  >
                  ...
                  {#if aiMoveStats.length > 0}
                    {#if aiMoveStats.find((stat) => stat.move === moveKey)}
                      {@const stat = aiMoveStats.find(
                        (stat) => stat.move === moveKey,
                      )}
                      <div
                        class="mb-3 text-xs bg-gray-800 text-white p-2 rounded"
                      >
          ...
          <!-- Player Fighter -->
          <div
            class="p-4 rounded-md mb-4 shadow-md bg-blue-50 border border-blue-300 text-black"
          >
            <div class="flex justify-between items-center mb-2">
              <h3 class="font-bold text-lg">{battleState.fighter1.name}</h3>
              <span class="text-sm"
                >HP: {battleState.fighter1.hp}/{battleState.fighter1
                  .maxHp}</span
              >
            </div>
            ...
            <div class="w-full bg-gray-200 h-4 mb-3 rounded-full">
              <div
                ...
                style="width: {Math.max(
                  ...
                  Math.min(
                    100,
                    (battleState.fighter1.hp / battleState.fighter1.maxHp) *
                      100,
                  ),
                )}%"
              ></div>
            ...
            <div class="grid gap-2">
              {#each Object.entries(battleState.fighter1.moves) as [moveKey, move]}
                <button
                  class="p-2 text-white rounded-md w-full mb-2 transition-all {battleState.battleOver
                    ? 'bg-blue-300'
                    : 'bg-blue-500 hover:bg-blue-600'} {selectedMove === moveKey
                    ? 'ring-2 ring-yellow-400'
                    : ''}"
                  disabled={battleState.battleOver ||
                    gameState !== GAME_STATES.WAITING_FOR_PLAYER}
                  onclick={() => handleMoveSelect(moveKey)}
                >
                  <div class="font-bold">{move.name}</div>
                  <div class="text-xs mt-1">{move.description}</div>
                </button>
        ...
        <!-- Battle Log -->
        <div
          class="bg-gray-800 text-gray-200 rounded-md p-3 h-48 overflow-y-auto mb-4 text-sm"
        >
